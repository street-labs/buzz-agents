...
setup:
    ...
    mkdir -p ~/.buzz/agents
    install -m 755 agent-watcher.sh harness-adapters.sh new-agent.sh warm-slots.sh agent-job.sh jev-triage.py jev-stall.py salon-arbiter.py ~/.buzz/agents/
...
uninstall:
    rm -f ~/.buzz/agents/{agent-watcher.sh,harness-adapters.sh,new-agent.sh,warm-slots.sh,agent-job.sh,jev-triage.py,jev-stall.py,salon-arbiter.py,base-agent-prompt.md,example.env}
