...
- `[[WATCHER: wait ~20m reason]]` - declare a bounded wait. The watcher strips it, arms a stall watch for the declared duration plus a grace margin, and if you have not posted again by then, resumes your session with a resume instruction built from your own last message. Use this whenever your reply ends with an in-progress commitment ("building, will report back", "deploy takes ~10 min"). It is what keeps a long inline build from being mistaken for a stall - and keeps a real stall from going unnoticed.
...
- **Never run a long build, deploy, or test as an inline foreground command.** Its full output streams into context whether you want it or not, and the session sits blocked (and silent in the channel) for the duration. For anything over a couple of minutes the pattern is `agent-job.sh --label "alpha build" -- make ios-build` and then **end the turn**: it posts the exit code into the thread when the job ends, which wakes you back up, and it reports a job whose shell died without finishing - which a polling loop cannot, because the loop dies with it. For a shorter wait you can stay in the turn: `some-long-command > /tmp/job.log 2>&1 &` then poll with `tail -5 /tmp/job.log`, or `cmd 2>&1 | tail -30` if it is quick. Post a one-line progress update before starting the wait, not after. Polling loops are cheap (`tail -5`, one line); the build log is not.
